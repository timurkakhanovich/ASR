...
class ASRProcessor:
    ...
    def __call__(
        ...
        log_mel_spec_image = torch.log(
            self.mel_spec_processor(input_values) + 1e-6
        ).clamp_(-10, 6)
        input_preprocessed = (
            self.augmentation(log_mel_spec_image).squeeze(0)
            if self.augmentation
            else log_mel_spec_image
        )

        text_preprocessed = self.text_processor(labels)

        return {'input_features': input_preprocessed, 'labels': text_preprocessed}
